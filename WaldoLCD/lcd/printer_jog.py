...
# @Last Modified by:   BH
# @Last Modified time: 2018-10-15 13:12:29
...
class Printer_Jog():
    def __init__(self):
        ...
        waldoprinter.printer_instance._logger.info("Initialized  Printer Jog, Octoprint Version is: " + str(self.octoprint_version))
    ...
    def jog(self,desired={'x':0, 'y':0, 'z':0, 'e':0 }, speed=None, relative=True, **kwargs):
        ...
        if self.octoprint_version <= "1.2.9":
            #use the old jog
            for axis in desired:
                waldoprinter.printer_instance._printer.jog(axis, desired[axis])

        elif self.octoprint_version >= "1.3.0":
            #use the new jog
            waldoprinter.printer_instance._printer.jog(axes=desired, speed=speed, relative=relative)
