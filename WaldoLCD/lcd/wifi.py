...
class AP_Mode():
    ...
    ap_mode_start = False
    ap_mode_fail = False
    def __init__(self, waldosm, name, **kwargs):
        self.sm = waldosm
        self.name = name #name of initial screen
        self.title = waldoprinter.lang.pack['WiFi']['AP_Title']
        self.ap_mode_1_4()
...
class APConfirmation(Label):
    text = StringProperty(waldoprinter.lang.pack['WiFi']['Initiating'])
...
class WifiButton(Button):
    ...
    def format(self):
        ...
        if self.ssid == '':
            self.ssid = waldoprinter.lang.pack['WiFi']['Empty_SSID']
...
class WifiConfiguration(Widget):
    ...
    wifi_status = 'nothing'
    counter = 0
    def __init__(self, waldoscreenmanager, back_destination, *args, **kwargs):
        # generate init screen: aka wifi list screen
        super(WifiConfiguration, self).__init__()
        self.rsm = waldoscreenmanager
    ...
    def generate_wifi_list_screen(self):
        ...
        self.rsm._generate_backbutton_screen(
            name=self.name,
            title=waldoprinter.lang.pack['WiFi']['Select_Network'],
