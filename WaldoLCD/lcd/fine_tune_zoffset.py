...
class Fine_Tune_ZOffset(object):
    ...
    def welcome_screen(self):
        layout = Button_Screen(waldoprinter.lang.pack['FT_ZOffset_Wizard']['Welcome']['Body'], 
    ...
    def user_set_mode(self):
        ...
        back_destination = 'welcome_fine_tune'
        if waldoprinter.waldosm.has_screen('welcome_fine_tune'):
            back_destination = 'welcome_fine_tune'
        elif waldoprinter.waldosm.has_screen('text_instructions'):
    ...
    def name_generator(self):
        if waldoprinter.waldosm.has_screen('pick_a_corner'):
            has_name = True
            name = 'pick_a_corner'
            updated_name = ''
            counter = 0
            while has_name:
                ...
                has_name = waldoprinter.waldosm.has_screen(updated_name)
                counter += 1
    ...
    def check_for_valid_start(self):
        if self.prepared_printer:
            ...
            waldoprinter.printer_instance._printer.commands('M400')#Wait for all previous commands to finish (Clear the buffer)
            waldoprinter.printer_instance._printer.commands('G1 X'+ str(self.start_pos_x) + ' Y'+ str(self.start_pos_y) + ' F5000') # go to first corner
            self.instruction1()
        else:
            ...
            if start:
                ...
                ep = Error_Popup(waldoprinter.lang.pack['FT_ZOffset_Wizard']['Z_Error']['Title'], waldoprinter.lang.pack['FT_ZOffset_Wizard']['Z_Error']['Body'] + str(zoff) + waldoprinter.lang.pack['FT_ZOffset_Wizard']['Z_Error']['Body1'],callback=partial(waldoprinter.waldosm.go_back_to_main, tab='printer_status_tab'))
                ep.open()
    ...
    def _prepare_for_lines(self):
        ...
        if not self.prepared_printer and not self.preparing_in_progress:
            ...
            waldoprinter.printer_instance._printer.commands('M400')#Wait for all previous commands to finish (Clear the buffer)
            waldoprinter.printer_instance._printer.commands('M104 S' + str(self.extruder_setting)) #Set Temperature
            waldoprinter.printer_instance._printer.commands('M140 S' + str(self.bed_setting))
            waldoprinter.printer_instance._printer.commands('G28') #Home Printer
            waldoprinter.printer_instance._printer.commands('G29')
            waldoprinter.printer_instance._printer.commands('G1 X'+ str(self.start_pos_x) + ' Y'+ str(self.start_pos_y) + ' F5000') # go to first corner
            ...
            layout = Wait_Screen(self.instruction1, 
                                 waldoprinter.lang.pack['FT_ZOffset_Wizard']['Prepare_Lines']['Sub_Title'] , 
                                 waldoprinter.lang.pack['FT_ZOffset_Wizard']['Prepare_Lines']['Body'])
            title = waldoprinter.lang.pack['FT_ZOffset_Wizard']['Prepare_Lines']['Title']
            name = 'temp_wait'
            back_destination = 'welcome_fine_tune'
    
            self.prepare_screen = waldoprinter.back_screen(
                name=name,
                title=title,
                back_destination=back_destination,
                content=layout
            ...
            waldoprinter.waldosm.add_widget(self.prepare_screen)
            waldoprinter.waldosm.current = 'temp_wait'
    ...
    def _line(self):
        ...
        waldoprinter.printer_instance._printer.commands('G1 E6.00 F500') #extrude filament
        waldoprinter.printer_instance._printer.commands('G1 Y' + str(self.travel_amount) + ' E15.00 F500') #make a line
        waldoprinter.printer_instance._printer.commands('G1 Z5 E10.00 F500') #Retract filament and pull nozzle off bed          
        ...
        if self.mode == "L2R":
            ...
            self.x -= 10
        #Goto next point
        waldoprinter.printer_instance._printer.commands('G1 X'+ str(self.x) + ' Y'+ str(self.start_pos_y) + ' F5000') #go to the next line start position
    ...
    def warn_and_restart(self):
        ...
        waldoprinter.printer_instance._printer.commands('G1 Z'+ str(drop) + ' F5000')
        waldoprinter.printer_instance._printer.commands('G1 X'+ str(self.start_pos_x) + ' Y'+ str(self.start_pos_y) + ' F5000') # go to first corner
        layout = Button_Screen(waldoprinter.lang.pack['FT_ZOffset_Wizard']['Warn_Restart']['Body'] , self.restart)
    ...
    def update_home_offset(self):
        ...
        title = waldoprinter.lang.pack['FT_ZOffset_Wizard']['Update_HO']['Title']
        name = 'home_adjust'
    ...
    def choose_to_finish(self):
        from bed_calibration_wizard import Modal_Question, Wait_Screen
        layout = Modal_Question(waldoprinter.lang.pack['FT_ZOffset_Wizard']['Choose_Finish']['Sub_Title'], 
                                waldoprinter.lang.pack['FT_ZOffset_Wizard']['Choose_Finish']['Body'], 
                                waldoprinter.lang.pack['FT_ZOffset_Wizard']['Choose_Finish']['Option1'],
                                waldoprinter.lang.pack['FT_ZOffset_Wizard']['Choose_Finish']['Option2'],
                                self.user_set_mode,
                                self.finish_wizard)

        title = waldoprinter.lang.pack['FT_ZOffset_Wizard']['Choose_Finish']['Title']
    ...
    def finish_wizard(self):
        ...
        layout = Picture_Button_Screen(waldoprinter.lang.pack['FT_ZOffset_Wizard']['Finish']['Sub_Title'], 
                                      waldoprinter.lang.pack['FT_ZOffset_Wizard']['Finish']['Body'] + str(offset),
...
class Update_Offset(BoxLayout):
    ...
    def set_offset(self):
        ...
        waldoprinter.printer_instance._printer.commands('M206 Z' + str(updated_offset))
    ...
    def add_offset(self):
        ...
        self.ids.offset_text.text = '[size=27]' + waldoprinter.lang.pack['FT_ZOffset_Wizard']['Update_Offset']['Body'] + '\n[/size][size=40][color=#69B3E7]' + str(self.actual_offset) + '[/size]'
