...
# @Last Modified by:   BH
# @Last Modified time: 2018-10-15 13:46:23
...
class PConsole(octoprint.printer.PrinterCallback, object):
    ...
    def on_printer_add_message(self, data):
        ...
        ##waldoprinter.printer_instance._logger.info(data)
        ...
        if re.match(printer_bed_error, data) or re.match(printer_bed_error2,data) or re.match(general_error, data) or re.match(connection_error,data):
            waldoprinter.printer_instance._logger.info("Disconnecting")
            waldoprinter.printer_instance._printer.disconnect()
            time.sleep(2)
            waldoprinter.printer_instance._logger.info("Reconnecting")
        ...
        if model == "Robo R2":
            ...
            def find_temps():
                ...
                #Single Nozzle R2
                    ...
                    waldoprinter.printer_instance._logger.info("Model is R2 and we cannot find bed and extruder!!!")
                    waldoprinter.printer_instance._logger.info(data)
            ...
            if data.find('T:') != -1 and data.find('B:') != -1:
                ...
                if float(self.temperature['bed']) < 0:
                    self.t_counter -= 1
                    waldoprinter.printer_instance._logger.info(str(self.t_counter))
                    if self.t_counter == 0:
                        waldoprinter.printer_instance._logger.info("Shutting down")
                        waldoprinter.printer_instance._printer.disconnect()
                        self.t_counter = 2
        ...
        if self.sent_M114:
            ...
            if temp_pos != []:
                ...
                waldoprinter.printer_instance._logger.info("position getting it in " + str(finished_time) + " ms")

                self.position = temp_pos[0]
                #waldoprinter.printer_instance._logger.info('Position Update')
                #waldoprinter.printer_instance._logger.info(str(self.position))
    ...
    def dict_logger(self, dictionary, indent = 0):
        ...
        for item in dictionary:
            if type(dictionary[item]) is dict:
                if indent == 0:
                    waldoprinter.printer_instance._logger.info("")
                waldoprinter.printer_instance._logger.info(indent_string + item + ":")
                self.dict_logger(dictionary[item], indent=(indent+1))
            else:
                report_string = indent_string + str(item) + ": " + str(dictionary[item])
                waldoprinter.printer_instance._logger.info(report_string)
    
    #Steps Per Unit
    def find_M92(self, data):
        #waldoprinter.printer_instance._logger.info("M92 "+ str(self.counter))
        self.steps_per_unit = self.merge_dicts(self.steps_per_unit,self.parse_M_commands(data, 'M92'))
        finished_time = (time.time() - self.cur_time) * 1000
        waldoprinter.printer_instance._logger.info("M92 getting it in " + str(finished_time) + " ms")

    #Maximum Feed Rate
    def find_M203(self, data):
        #waldoprinter.printer_instance._logger.info("M203 "+ str(self.counter))
        self.feed_rate = self.merge_dicts(self.feed_rate, self.parse_M_commands(data, "M203"))
        finished_time = (time.time() - self.cur_time) * 1000
        waldoprinter.printer_instance._logger.info("M203 getting it in " + str(finished_time) + " ms")
    ...
    def find_M201(self, data):
        ...
        finished_time = (time.time() - self.cur_time) * 1000
        waldoprinter.printer_instance._logger.info("M201 getting it in " + str(finished_time) + " ms")

    #Accelerations
    def find_M204(self, data):
        #waldoprinter.printer_instance._logger.info("M204 "+ str(self.counter))
        
        self.accelerations = self.merge_dicts(self.accelerations, self.parse_M_commands(data, "M204"))
        finished_time = (time.time() - self.cur_time) * 1000
        waldoprinter.printer_instance._logger.info("M204 getting it in " + str(finished_time) + " ms")

    #advanced variables
    def find_M205(self, data):
        #waldoprinter.printer_instance._logger.info("M205 "+ str(self.counter))
        
        self.advanced_variables = self.merge_dicts(self.advanced_variables, self.parse_M_commands(data, "M205"))
        finished_time = (time.time() - self.cur_time) * 1000
        waldoprinter.printer_instance._logger.info("M205 getting it in " + str(finished_time) + " ms")

    #home offset
    def find_M206(self, data):
        self.home_offset = self.merge_dicts(self.home_offset, self.parse_M_commands(data, "M206"))
        finished_time = (time.time() - self.cur_time) * 1000
        waldoprinter.printer_instance._logger.info("M206 getting it in " + str(finished_time) + " ms")

    #hotend offset
    def find_M218(self, data):
        self.hotend_offset = self.merge_dicts(self.hotend_offset, self.parse_M_commands(data, "M218"))
        finished_time = (time.time() - self.cur_time) * 1000
        waldoprinter.printer_instance._logger.info("M118 getting it in " + str(finished_time) + " ms")


    #PID settings
    def find_M301(self, data):
        #waldoprinter.printer_instance._logger.info("M301 " + data)
        self.PID = self.merge_dicts(self.PID, self.parse_M_commands(data, 'M301'))
        finished_time = (time.time() - self.cur_time) * 1000
        waldoprinter.printer_instance._logger.info("M301 getting it in " + str(finished_time) + " ms")

    def find_M304(self, data):
        #waldoprinter.printer_instance._logger.info("M301 "+ str(self.counter))
        
        self.BPID = self.merge_dicts(self.BPID, self.parse_M_commands(data, "M304"))
        finished_time = (time.time() - self.cur_time) * 1000
        waldoprinter.printer_instance._logger.info("M304 getting it in " + str(finished_time) + " ms")
    ...
    def find_M851(self, data):
        ...
        finished_time = (time.time() - self.cur_time) * 1000
        waldoprinter.printer_instance._logger.info("M851 getting it in " + str(finished_time) + " ms")
        #EEPROM is ready
    ...
    def find_zoffset(self,data):
        ...
        self.probe_offset = self.merge_dicts(self.probe_offset, self.parse_M_commands(data, "M851"))
        finished_time = (time.time() - self.cur_time) * 1000
        waldoprinter.printer_instance._logger.info("ZOffset getting it in " + str(finished_time) + " ms")
    ...
    def find_M900(self, data):
        ...
        finished_time = (time.time() - self.cur_time) * 1000
        waldoprinter.printer_instance._logger.info("M900 getting it in " + str(finished_time) + " ms")
    ...
    def change_tool(self, tool):
        ...
        if not self.busy:
            if tool in acceptable_tools:
                ...
                waldoprinter.printer_instance._printer.commands(acceptable_tools[tool])
    ...
    def register_observer(self, command, callback):
        ...
        if command in pconsole_values:
            if command in self.registered_callbacks and type(self.registered_callbacks[command]) == list:
                ...
                waldoprinter.printer_instance._logger.info("Added a callback for command: " + str(command) + " ID: " + str(id(callback)))
            else:
                self.registered_callbacks[command] = [callback]
                waldoprinter.printer_instance._logger.info("Added first callback for command: " + str(command) + " ID: " + str(id(callback)))
            return True
        waldoprinter.printer_instance._logger.info("Failed to add callback for command: " + str(command) + " ID: " + str(id(callback)))
        return False
    ...
    def unregister_observer(self, command, callback):
        ...
        if command in pconsole_values:
            ...
            if callback in self.registered_callbacks[command]:
                ...
                waldoprinter.printer_instance._logger.info("Deleted Callback for command: " + str(command) + " ID: " + str(id(callback)))
                return True
        waldoprinter.printer_instance._logger.info("Failed to remove callback for command: " + str(command) + " ID: " + str(id(callback)))
        return False
    ...
    def observer_caller(self, var_id, value):
        if var_id in self.registered_callbacks:
            #import json
            #waldoprinter.printer_instance._logger.info(str(self.registered_callbacks))
    ...
    def join_EEPROM(self, join_list = []):
        ...
        def catcher(command, value):
            waldoprinter.printer_instance._logger.info("Command: " + str(command) + " Returned: " + str(value))
            catcher_returns[command] = value
            
        def monitor_returns():
            import time
            returns_complete = False
            waldoprinter.printer_instance._logger.info("Thread starting")
            while not returns_complete:
                if len([x for x in join_list if (x in catcher_returns)]) == len(join_list):
                    ...
                    waldoprinter.printer_instance._logger.info("Returns Complete")
                    break
                else:
                    time.sleep(1)
