...
from waldo_controls import Temperature_Label
...
class Scroll_Box_Icons(GridLayout):
    ...
    buttons = []
    def __init__(self, button_array, waldosm=None, **kwargs):
    ...
    def populate_buttons(self):
        ...
        if length == 5 or length == 3:
            if self.sm != None:
                tl = Temperature_Label(waldosm=self.sm)
                content.add_widget(tl)
            else:
                content.add_widget(Button(text='', background_color = [0,0,0,1],size_hint = [0.0,0.0] ))

class Waldo_Icons(Button):
    generator= StringProperty("WALDO_CONTROLS")
    img_source = StringProperty("Icons/Icon_Buttons/Waldo_Controls.png")
    icon_name = StringProperty("Waldo Controls")
    ...
    def execute_function(self):
        ...
        self.current_screen = waldoprinter.waldosm.current
    ...
    def Icon_Loading(self, dt):
        ...
        if self.current_screen != waldoprinter.waldosm.current:
            ...
            return False
'''
Same as Waldo Icons, the kivy file is different though
'''
...
class Waldo_Icons_Anchor(Button):
    generator= StringProperty("WALDO_CONTROLS")
    img_source = StringProperty("Icons/Icon_Buttons/Waldo_Controls.png")
    icon_name = StringProperty("Waldo Controls")
    ...
    def __init__(self, _image_source, _icon_name, _generator_function, position):
        super(Waldo_Icons_Anchor, self).__init__()
        self. generator = _generator_function
        self.img_source = _image_source
        self.icon_name = _icon_name
