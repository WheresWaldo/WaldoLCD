...
# @Last Modified by:   BH
# @Last Modified time: 2018-10-15 13:14:51
...
class Print_Tuning(BoxLayout):
    ...
    def flow_button(self):
        ...
        self.title_label_text = "[size=40][color=#69B3E7]" + waldoprinter.lang.pack['Print_Tuning']['Flow_Rate'] + "[/size][/color]"
        self.save_tuning_info()

    def feed_button(self):
        self.set_active('FEED')
        self.title_label_text = "[size=40][color=#69B3E7]" + waldoprinter.lang.pack['Print_Tuning']['Printing_Speed'] + "[/size][/color]"
        self.save_tuning_info()

    def fan_button(self):
        self.set_active('FAN')
        self.title_label_text = "[size=40][color=#69B3E7]" + waldoprinter.lang.pack['Print_Tuning']['Fan_Speed'] + "[/size][/color]"
        self.save_tuning_info()
    ...
    def apply_tunings(self):
        if self.current_tuner == 'FLOW':
            waldoprinter.printer_instance._printer.flow_rate(self.tune_rates['FLOW'])
        elif self.current_tuner == 'FEED':
            waldoprinter.printer_instance._printer.feed_rate(self.tune_rates['FEED'])
            ...
            waldoprinter.printer_instance._printer.commands('M106 S'+ str(fan_percentage_speed))
