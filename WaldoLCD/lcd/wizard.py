...
class FilamentWizard(Widget):
    ...
    layout2 = None
    extrude_event = None
    def __init__(self, loader_changer, waldosm, name,  **kwargs):
    ...
    def second_screen(self, *args):
        ...
        if not self.is_printing and not self.is_paused:
            ...
            waldoprinter.printer_instance._printer.set_temperature('tool0', self.print_temperature)
    ...
    def update_temp_label(self, obj, *args):
        ...
        obj.text = str(self.temp) + waldoprinter.lang.pack['Filament_Wizard']['Celsius_Alone']

    def poll_temp(self, *args):
        # updates the temperature
        r = waldoprinter.printer_instance._printer.get_current_temperatures()
    ...
    def third_screen(self):
        ...
        """
        # waldoprinter.printer_instance._printer.jog('e', -130.00)
        c = Filament_Wizard_3_5(self.fourth_screen)
        back_destination = waldoprinter.waldo_screen()
        this_screen = self.sm._generate_backbutton_screen(name=self.name+'[2]', title=waldoprinter.lang.pack['Filament_Wizard']['Title_25'], back_destination=back_destination, content=c)
        ...
        waldoprinter.printer_instance._printer.extrude(20.0)
    ...
    def extrude(self, *args):
        ...
        if self.sm.current == 'filamentwizard[4]':
            waldoprinter.printer_instance._printer.extrude(5.0)
        else:
            self.end_extrude_event()
    ...
    def retract_after_session(self, *args):
        waldoprinter.printer_instance._printer.extrude(-10.0)
    ...
    def end_wizard(self, *args):
        ...
        if self.E_Position != None:
            waldoprinter.printer_instance._printer.commands("G92 E" + str(self.E_Position))        
