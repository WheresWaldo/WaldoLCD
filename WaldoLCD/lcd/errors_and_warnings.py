...
class Refresh_Screen(Title_Button_Screen):
    ...
    def soft_reset(self):
        ...
        if status.find("Offline") != -1:
            status = waldoprinter.lang.pack['Refresh_Screen']['Soft_Reset']['Status']

        reset_title = waldoprinter.lang.pack['Refresh_Screen']['Soft_Reset']['Title']
        body_text = (waldoprinter.lang.pack['Refresh_Screen']['Soft_Reset']['Please'] + str(self.reconnect_choice).lower()
                     + waldoprinter.lang.pack['Refresh_Screen']['Soft_Reset']['Body'] 
    ...
    def reset(self):
        self.changed_text = False
        self.title = str(self.reconnect_choice) + waldoprinter.lang.pack['Refresh_Screen']['Reset']['Title']
        self.body_text = waldoprinter.lang.pack['Refresh_Screen']['Reset']['Body']

        def reconnect(dt):
            waldoprinter.printer_instance._printer.disconnect()
            if self.clock_monitor == None:
                self.clock_monitor = Clock.schedule_interval(self.update_connection_status, 0.2)
            else:
    ...
    def connect(self, dt):
        waldoprinter.printer_instance._printer.connect()

    def slow_operational(self, dt):
        current_data = waldoprinter.printer_instance._printer.get_current_data()
        status = current_data['state']['text']

        self.title_text = str(self.reconnect_choice) + waldoprinter.lang.pack['Refresh_Screen']['Update_Connection']['Successful']
    ...
    def slow_disconnect(self, dt):
        ...
        self.title_text = str(self.reconnect_choice) + waldoprinter.lang.pack['Refresh_Screen']['Update_Connection']['In_Progress']
    ...
    def error_report(self, status):
        ...
        self.title_text = waldoprinter.lang.pack['Refresh_Screen']['Soft_Reset']['Title']
        self.body_text = (waldoprinter.lang.pack['Refresh_Screen']['Soft_Reset']['Please'] + str(self.reconnect_choice).lower()
                     + waldoprinter.lang.pack['Refresh_Screen']['Soft_Reset']['Body'] 
                     + '[color=FF0000]'  + status.replace("Error:","").strip() + '[/color]'
                     )
        if self.clock_monitor != None:
    ...
    def update_connection_status(self, dt):
        ...
        if status == 'Operational':
            ...
            if not self.bed_disconnect:
                ...
                self.title_text = waldoprinter.lang.pack['Refresh_Screen']['Update_Connection']['bed_connect']
                if self.bed_checker != None:
                    self.bed_checker.cancel()
        ...
        if status.find("Offline") != -1:
            self.body_text = waldoprinter.lang.pack['Refresh_Screen']['Update_Connection']['offline']
        else:
            self.body_text = status

        current_screen = str(waldoprinter.waldosm.current) 
...
class Firmware_Upgrade(Picture_Button_Screen):
    ...
    def __init__(self):
        self.title = waldoprinter.lang.pack['Firmware']['Title']
        self.body = waldoprinter.lang.pack['Firmware']['Body']
...
class Bed_Heating(Picture_Button_Screen):
    ...
    def __init__(self):
        self.title = waldoprinter.lang.pack['Bed_Hot']['Title']
        self.body = waldoprinter.lang.pack['Bed_Hot']['Body']
...
class Error_Detection(Button):
    ...
    def populate_error(self, error):
        ...
        acceptable_errors = {
            'MAINBOARD': {
                'title': waldoprinter.lang.pack['Error_Detection']['MAINBOARD']['Title'],
                'body' : waldoprinter.lang.pack['Error_Detection']['MAINBOARD']['Body'],
                'icon' : "Icons/Printer Status/main board warning.png",
                'function': self.main_board_disconnect,
                'caret': True
            },
            'FIRMWARE':{
                'title': waldoprinter.lang.pack['Error_Detection']['FIRMWARE']['Title'],
                'body' :waldoprinter.lang.pack['Error_Detection']['FIRMWARE']['Body'],
                'icon' : "Icons/Printer Status/Firmware warning.png",
                'function': self.firmware_updating,
                'caret': True,
            },
            'BED_HOT':{
                'title': waldoprinter.lang.pack['Error_Detection']['BED_HOT']['Title'],
                'body' : waldoprinter.lang.pack['Error_Detection']['BED_HOT']['Body'],
                'icon' : "Icons/Printer Status/bed heating.png",
                'function': self.bed_hot_warning,
                'caret': True,
            },
            'BED_DISCONNECT':{
                'title': waldoprinter.lang.pack['Error_Detection']['BED_DISCONNECT']['Title'],
                'body' : waldoprinter.lang.pack['Error_Detection']['BED_DISCONNECT']['Body'],
                'icon' : "Icons/Printer Status/bed warning.png",
                'function': self.bed_disconnect_screen,
                'caret': True,
            },
            'PAUSED':{
                'title': waldoprinter.lang.pack['Error_Detection']['PAUSED']['Title'],
                'body' : "[color=#69B3E7]" + waldoprinter.lang.pack['Error_Detection']['PAUSED']['Body'],
                'icon' : "Icons/Printer Status/Pause.png",
                'function': self.placeholder,
                'caret': False
            },
            'FIL_RUNOUT':{
                'title': waldoprinter.lang.pack['Error_Detection']['FIL_RUNOUT']['Title'],
            ...
            'DEFAULT':{
                'title': waldoprinter.lang.pack['Error_Detection']['DEFAULT']['Title_Idle'] if not waldoprinter.printer_instance._printer.is_printing() else waldoprinter.lang.pack['Error_Detection']['DEFAULT']['Title_Active'],
                'body' : "[color=#69B3E7]" + waldoprinter.lang.pack['Error_Detection']['DEFAULT']['Body_Idle'] + "[/color]" if not waldoprinter.printer_instance._printer.is_printing() else "[color=#69B3E7]" + waldoprinter.lang.pack['Error_Detection']['DEFAULT']['Body_Active'] + "[/color]",
    ...
    def bed_disconnect_screen(self):
        ...
        title = '[color=FF0000]' + waldoprinter.lang.pack['Error_Detection']['BED_DISCONNECT']['E_Sub_Title'] + '[/color]'
        body_text = waldoprinter.lang.pack['Error_Detection']['BED_DISCONNECT']['E_Body']
        button_text = waldoprinter.lang.pack['Error_Detection']['BED_DISCONNECT']['E_Button']
            
        layout = Refresh_Screen(title, body_text, button_text, bed_disconnect=True)

        title = waldoprinter.lang.pack['Error_Detection']['BED_DISCONNECT']['E_Title']
        name = 'mainboard'
    ...
    def firmware_updating(self):
        ...
        title = waldoprinter.lang.pack['Error_Detection']['FIRMWARE']['E_Title']
    ...
    def main_board_disconnect(self):
        ...
        if current_screen != 'mainboard' and current_screen != 'mainboard_status':
            ...
            status = current_data['state']['text']
            if status.find("Offline") != -1:
                status = waldoprinter.lang.pack['Error_Detection']['MAINBOARD']['Connection_Offline']
            elif status.find("Printing") != -1:
                while status == "Printing":
                    current_data = waldoprinter.printer_instance._printer.get_current_data()
                    status = current_data['state']['text']

            reset_title = waldoprinter.lang.pack['Error_Detection']['MAINBOARD']['E_Sub_Title']
            body_text = (waldoprinter.lang.pack['Error_Detection']['MAINBOARD']['E_Body'] + '[color=FF0000]'  + status.replace("Error:","").strip() + '[/color]'
                     )
            button_text = waldoprinter.lang.pack['Error_Detection']['MAINBOARD']['E_Button']
            
            layout = Refresh_Screen(reset_title, body_text, button_text)
    
            title =waldoprinter.lang.pack['Error_Detection']['MAINBOARD']['E_Title']
            name = 'mainboard'
            back_destination = 'main'
    ...
    def fil_runout(self):
        ...
        title = waldoprinter.lang.pack['Error_Detection']['FIL_RUNOUT']['E_Title']
        name = 'fil_runout'
        back_destination = 'main'
    ...
    def check_connection_status(self, dt):
        ...
        is_closed_or_error = waldoprinter.printer_instance._printer.is_closed_or_error()
        is_updating_firm = waldoprinter.printer_instance.firmware_updating()
        is_printing = waldoprinter.printer_instance._printer.is_printing()
        is_paused = waldoprinter.printer_instance._printer.is_paused()
        auto_pause = False

        #get the filament status
        if waldoprinter.printer_instance.check_auto_pause != None:
            auto_pause = waldoprinter.printer_instance.check_auto_pause()
            if auto_pause:
        ...
        if waldoprinter.printer_instance._printer.get_current_connection()[0] == 'Closed':
            is_closed = True
        else:
            is_closed = False

        is_error = waldoprinter.printer_instance._printer.is_error()
